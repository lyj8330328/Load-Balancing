...
服务消费方进行熔断，当遇到请求不到的服务，不能一直等待下去，要实行熔断机制。
...
![](G:\SSM\ssm\乐优\day03-springcloud\assets\1525662976679.png)
...
![](G:\SSM\ssm\乐优\day03-springcloud\assets\1525672466192.png)
...
**不要使用requestMapping("user")，会产生冲突，把当前接口当作controller了。**
...
![](G:\SSM\ssm\乐优\day03-springcloud\assets\1525675037152.png)
...
- `filterType`：返回字符串，代表过滤器的类型。包含以下4种：
  - `pre`：请求在被路由之前执行         前置：在所有请求被分发前执行
  - `routing`：在路由请求时调用         路由
  - `post`：在routing和errror过滤器之后调用    后置：服务响应后执行
  - `error`：处理请求时发生错误调用
...
![](assets\1525683285697.png)

添加token参数后：

![](assets\1525683354113.png)
